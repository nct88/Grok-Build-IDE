...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	constructor(
		...
		//#region create browser window
		{
			...
			if (isWindows && this.productService.win32AppUserModelId) {
				const appId = `${this.productService.win32AppUserModelId}.Process${process.pid}.Window${this._id}`;
				const applyAppId = () => this._win?.setAppDetails({ appId });
				applyAppId();
				this._win.on('show', applyAppId);
			}
