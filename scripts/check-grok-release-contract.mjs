...
const failures = [];
if (version !== extension.version) failures.push(`version mismatch: release=${version}, extension=${extension.version}`);
...
if (/H:\\projects/i.test(activeScripts)) failures.push("active release scripts contain a fixed H:\\projects path");
if (/Invoke-Retention|Clear-OldBuilds/.test(activeScripts)) failures.push("active release scripts must not prune releases or builds");
if (!/Required release artifact is missing/.test(activeScripts)) failures.push("publisher must fail on incomplete channels");
if (!/PublicRelease requires an HTTPS/.test(activeScripts)) failures.push("public release must require HTTPS");
if (!/Get-AuthenticodeSignature/.test(activeScripts)) failures.push("public release must verify signatures");
if (!/Grok-Build-IDE-\$Version-win32-x64-portable/.test(activeScripts)) failures.push("portable artifact name is not branded");
if (!/height:\s*100%/.test(read("extensions/grok-build-workbench/media/styles.css"))) failures.push("webview app must fill viewport height");
...
console.log(`Release contract OK (${version}): branded, immutable, complete channels, signed HTTPS public gate.`);
